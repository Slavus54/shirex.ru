...
import {getOrganizationsQ} from './gql'
...
const Organizations: React.FC = () => {
    ...
    useMemo(() => {
        if (organizations !== null) {
            let result: any[] = organizations.filter(el => el.region === region)
        ...
        <ComponentLoadingWrapper 
            component={
                <>
                    ...
                    <h5 className='pale'>Выберите тип и масштаб учереждения</h5>

                    <div className='items small'>
                        {ORGANIZATION_TYPES.map(el => <div onClick={() => setCategory(el)} className={classHandler(el, category)}>{el}</div>)}
                    </div>  
                    ...
                    <ReactMapGL mapStyle={onInitMapStyle()} onClick={e => setCords(codus.cords(e))} {...view} onViewportChange={(e: any) => setView(e)} mapboxApiAccessToken={token}>
                        ...
                        {filtered.map(el => 
                            <Marker latitude={el.cords.lat} longitude={el.cords.long}>
                                <RouterNavigator url={`/organization/${el.shortid}`} isOnMap={true}>{codus.short(el.title)}</RouterNavigator>
                            </Marker>
                        )}
                    </ReactMapGL> 
                </>
            ...
            isLoading={loading}
            text='Список организаций загружается'
        />
    )
}
