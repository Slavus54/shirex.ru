...
import {getCompaniesQ} from './gql'
import {SEARCH_PERCENT, VIEW_CONFIG, MAP_ZOOM, token} from '@/env/env'
import {COMPANY_TYPES, COMPANY_FORMATS} from './env'
...
const Companies: React.FC = () => {
    ...
    useMemo(() => {
        if (companies !== null) {
            let result: any[] = companies.filter(el => el.region === region)
    ...
    }, [companies, title, category, format, region])
    ...
    return (
        <ComponentLoadingWrapper 
            component={
                <>
                    ...
                    <div className='items small'>
                        {COMPANY_TYPES.map(el => <div onClick={() => setCategory(el)} className={classHandler(el, category)}>{el}</div>)}
                    </div>  
                    ...
                    <DataPagination items={filtered} setItems={setFiltered} label='Компании:' />
                    ...
                    <ReactMapGL mapStyle={onInitMapStyle()} onClick={e => setCords(codus.cords(e))} {...view} onViewportChange={(e: any) => setView(e)} mapboxApiAccessToken={token}>
                        ...
                        {filtered.map(el => 
                            <Marker latitude={el.cords.lat} longitude={el.cords.long}>
                                <RouterNavigator url={`/company/${el.shortid}`} isOnMap={true}>{codus.short(el.title)}</RouterNavigator>
                            </Marker>
                        )}
                    </ReactMapGL> 
                </>
            ...
            isLoading={loading}
            text='Список компаний загружается'
        />
    )
}
